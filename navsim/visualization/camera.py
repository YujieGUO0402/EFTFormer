...
def add_lidar_to_camera_ax(ax: plt.Axes, camera: Camera, lidar: Lidar) -> plt.Axes:
    ...
    # Resize 图像（注意 OpenCV 的尺寸顺序是 (width, height)）
    ...
    # 调整内参矩阵
    original_intrinsics = camera.intrinsics.copy()
    new_intrinsics = np.array([
        [original_intrinsics[0, 0]*scale_x, 0, target_width / 2],
        [0, original_intrinsics[1, 1]*scale_y, target_height / 2],
        [0, 0, 1]
    ])
    ...
    pc_in_cam, pc_in_fov_mask = _transform_pcs_to_images(
        ...
        camera.sensor2lidar_rotation,
        camera.sensor2lidar_translation,
        new_intrinsics,  # 使用调整后的内参
        img_shape=(target_height, target_width),  # 新尺寸
    )
    
    # 在调整后的图像上绘制点云
    for (x, y), color in zip(pc_in_cam[pc_in_fov_mask], lidar_pc_colors[pc_in_fov_mask]):
        color = (int(color[0]), int(color[1]), int(color[2]))
        cv2.circle(resized_image, (int(x), int(y)), 5, color, -1)
    ...
    ax.text(
        0.05, 0.8,
        f"Intrinsics:\n{np.round(new_intrinsics, 2)}",  # 显示调整后的内参
        color="white",
        fontsize=6,
        transform=ax.transAxes,
        bbox=dict(facecolor="black", alpha=0.8)
