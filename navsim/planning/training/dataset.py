...
class Dataset(torch.utils.data.Dataset):
    ...
    def _cache_scene_with_token(self, token: str) -> None:
        ...
        # 如果 scene 是 None（例如 occ_gt 缺失），则跳过
        if scene is None:
            print(f"[Warning] Skipping scene with token: {token} (scene is None)")
            with open("skipped_scenes_due_to_missing_occ.txt", "a") as f:
                f.write(f"{token}\n")
            return  # 跳过当前 scene 的缓存过程
